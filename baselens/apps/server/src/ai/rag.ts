...
export async function processRagChat(
  ...
  const context = await searchRagDocuments(analysisId, question);
  
  // Build context string
  const contextString = context.documents
    .map((doc) => `[${doc.kind}: ${doc.refId}]\n${doc.content}`)
    .join("\n\n---\n\n");
  
  logger.debug(`[RAG] Context string: ${contextString.length} chars from ${context.documents.length} documents`);
  ...
  const userPrompt = RAG_USER_PROMPT
    .replace("{context}", contextString || "No relevant context found.")
